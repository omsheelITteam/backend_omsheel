...
const register = async (req, res) => {
  ...
  try {
    ...
    let verifyOtp = "";
    for (let i = 0; i < 6; i++) {
      verifyOtp += Math.floor(Math.random() * 10);
    }
    ...
    // const token = jwt.sign({ id: user._id }, JWT_SECRETKEY, { expiresIn: "7d" });
    // res.cookie("token", token, {
    //   httpOnly: true,
    //   secure: process.env.NODE_ENV === "production",
    //   sameSite: process.env.NODE_ENV === "production" ? "none" : "lax",
...
const resetPassword = async (req, res) => {
  ...
  try {
    const user = await userModel.findOne({ email });
    if (!user || user.resetOtp !== code || user.resetOtpExpireAt < Date.now()) {
...
const sendResetOtp = async (req, res) => {
  const { email } = req.body;
  if (!email) return res.json({ success: false, message: "Invalid Credentials" });
  ...
  try {
    ...
    let resetOtp = "";
    for (let i = 0; i < 6; i++) resetOtp += Math.floor(Math.random() * 10);
